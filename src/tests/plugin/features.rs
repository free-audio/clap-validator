...
pub fn test_category_features(library: &PluginLibrary, plugin_id: &str) -> TestStatus {
    let result = library
        ...
        .and_then(|metadata| {
            ...
            let has_main_category = metadata.features.iter().any(|feature| -> bool {
            ...
            if has_main_category {
                ...
                anyhow::bail!(
                    "The plugin needs to have at least one of thw following plugin category \
                     features: \"{instrument_feature}\", \"{audio_effect_feature}\", \
